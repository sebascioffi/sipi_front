...
const MovieUsuario = () => {
  ...
  useEffect(() => {
    const obtenerFavoritasUsuario = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Error al obtener las películas favoritas del usuario');

        const data = await response.json();

        // Convertimos todas las favoritas a números antes de comparar
        const favoritasNumeros = data.favoritas.map(fav => Number(fav));

        setEnFavoritas(favoritasNumeros.includes(Number(id)));
      } catch (error) {
  ...
  useEffect(() => {
    const obtenerPendientesUsuario = async () => {
      ...
      try {
        const response = await fetch(url);
        if (!response.ok) throw new Error('Error al obtener las películas pendientes del usuario');

        const data = await response.json();

        // Convertimos todas las favoritas a números antes de comparar
        const pendientesNumeros = data.pendientes.map(pen => Number(pen));

        setEnPendientes(pendientesNumeros.includes(Number(id)));
      } catch (error) {
        console.error('Error:', error);
      }
    };
  ...
  const handleEliminarFavorita = () => {
    const url = `${port}/user/favorita/${nom_usuario}/${id}`;
  ...
  const handleEliminarPendiente = () => {
    const url = `${port}/user/pendiente/${nom_usuario}/${id}`;
